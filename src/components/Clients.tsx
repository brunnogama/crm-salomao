    ...
    <div className="h-full flex flex-col space-y-6 animate-fadeIn pb-10">
      ...
      <div className="bg-white/70 backdrop-blur-md p-4 rounded-2xl border border-white/40 shadow-sm flex flex-wrap gap-4 items-center justify-between">
        <div className="flex gap-3 items-center flex-1 min-w-[300px]">
          ...
          <button className="p-2 bg-white/50 border border-gray-100 rounded-xl hover:bg-white transition-colors"><Filter className="h-4 w-4 text-gray-500" /></button>
          <button onClick={fetchClients} className="p-2 bg-white/50 border border-gray-100 rounded-xl hover:bg-white transition-colors"><RefreshCcw className="h-4 w-4 text-gray-500" /></button>
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all"><Download className="h-4 w-4" /> EXPORTAR</button>
          <button className="flex items-center gap-2 bg-[#112240] text-white px-4 py-2 rounded-xl text-xs font-bold shadow-lg shadow-blue-900/20 hover:bg-black transition-all"><Plus className="h-4 w-4" /> NOVO CLIENTE</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto pr-2 custom-scrollbar">
        {clients.map((client) => (
          <div key={client.id} className="group bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-white/60 transition-all hover:shadow-xl hover:shadow-blue-900/[0.03] hover:-translate-y-1">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex flex-col items-end gap-2">
                <span className="text-[9px] font-black px-2 py-1 rounded-full border" style={{ color: getBrindeColor(client.tipo_brinde), borderColor: `${getBrindeColor(client.tipo_brinde)}30`, backgroundColor: `${getBrindeColor(client.tipo_brinde)}10` }}>
                  {client.tipo_brinde?.toUpperCase()}
                </span>
            ...
            <div className="space-y-3 pb-4 border-b border-gray-50">
              <div className="flex items-center gap-3 text-gray-500">
                ...
                <span className="text-[10px] font-bold uppercase tracking-tight">{client.socio}</span>
              </div>
              <div className="flex items-center gap-3 text-gray-500">
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <button className="text-[10px] font-black text-blue-600 hover:underline">VER DETALHES</button>
            </div>
          </div>
        ))}
      </div>
