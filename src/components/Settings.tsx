...
export function Settings() {
  ...
  // --- ACTIONS: SYSTEM (RESET MANUAL AGRESSIVO) ---

  const handleSystemReset = async () => {
    if (!confirm('PERIGO: Isso apagará TODOS os dados (Clientes e Magistrados) do sistema. Tem certeza absoluta?')) return;
    ...
    try {
        // 1. Apagar Tarefas primeiro (para evitar erro de FK)
        console.log("Apagando tarefas...");
        const { error: errTasks } = await supabase.from('tasks').delete().neq('id', 0)
        if (errTasks) console.error("Erro ao apagar tarefas (pode ser normal se estiver vazio):", errTasks);

        // 2. Apagar Magistrados
        console.log("Apagando magistrados...");
        const { error: errMag } = await supabase.from('magistrados').delete().neq('id', 0)
        if (errMag) throw new Error("Erro Magistrados: " + errMag.message)

        // 3. Apagar Clientes
        console.log("Apagando clientes...");
        const { error: errCli } = await supabase.from('clientes').delete().neq('id', 0)
        if (errCli) throw new Error("Erro Clientes: " + errCli.message)
