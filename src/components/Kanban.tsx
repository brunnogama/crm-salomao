    ...
    <div className="h-full flex flex-col">
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex-1 flex gap-6 overflow-x-auto pb-4">
          {COLUMNS.map((column) => (
            <div key={column.id} className={`flex-shrink-0 w-80 lg:w-96 flex flex-col rounded-xl border ${
              column.color === 'orange' ? 'bg-orange-50/50 border-orange-200' : 
              column.color === 'blue' ? 'bg-blue-50/50 border-blue-200' : 'bg-green-50/50 border-green-200'
            }`}>
              <div className="p-4 flex items-center justify-between">
                ...
                <span className="bg-white/50 px-2 py-0.5 rounded-full text-xs font-bold text-gray-600 border border-black/5">
              ...
              <div className="px-4 mb-2">
                <button 
                  onClick={() => { setNewTask({ ...newTask, status: column.id }); setIsAddModalOpen(true); }}
                  className={`w-full py-2 rounded-lg text-white font-medium flex items-center justify-center gap-2 transition-all active:scale-95 shadow-sm ${
              ...
              <Droppable droppableId={column.id}>
                {(provided, snapshot) => (
                  <div {...provided.droppableProps} ref={provided.innerRef} className={`flex-1 p-3 transition-colors ${snapshot.isDraggingOver ? 'bg-black/5' : ''}`}>
                      ...
                      <Draggable key={task.id} draggableId={task.id} index={index}>
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-3 hover:border-gray-300 transition-all group cursor-pointer ${
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <span className="bg-blue-50 text-blue-700 text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider">{task.priority}</span>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-scaleIn">
            ...
            <form onSubmit={handleAddTask} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" className="w-full border border-gray-200 rounded-lg px-4 py-2 outline-none focus:border-blue-500" value={newTask.title} onChange={e => setNewTask({...newTask, title: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Descrição</label>
                <textarea rows={3} className="w-full border border-gray-200 rounded-lg px-4 py-2 outline-none focus:border-blue-500 resize-none" value={newTask.description} onChange={e => setNewTask({...newTask, description: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Prioridade</label>
                <select className="w-full border border-gray-200 rounded-lg px-4 py-2 outline-none" value={newTask.priority} onChange={e => setNewTask({...newTask, priority: e.target.value})}>
                  <option value="BAIXA">Baixa</option>
                  <option value="MEDIA">Média</option>
                  <option value="ALTA">Alta</option>
                </select>
              </div>
              <button type="submit" className="w-full bg-[#112240] text-white py-3 rounded-xl font-bold hover:bg-[#1a3a6c] transition-colors shadow-lg">Criar Tarefa</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isDetailsModalOpen && selectedTask && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl animate-scaleIn">
            ...
            <div className="p-8">
              <div className="mb-6"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1 block">Título</label><h3 className="text-xl font-bold text-gray-800">{selectedTask.title}</h3></div>
              ...
              <div className="mt-6 text-xs text-gray-400 font-medium">CRIADO EM {new Date(selectedTask.created_at).toLocaleDateString()}</div>
            </div>
          </div>
        </div>
      )}
